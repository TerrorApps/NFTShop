...
export default async function handler(
  ...
  if (Number(tokenId) > 7777 || Number(tokenId) < 1) {
    ...
    await Jimp.read(imageUrl, async function (err, image) {
      ...
      var suit_background = path.resolve('.', 'assets/0n1/garou-mask_shadow.png')
    //   var background_suit = await sharp(suit_background).resize({
    //       fit: sharp.fit.contain,
    //       width: 1170,
    //       height: 1170,
    //       background: { r: rgb.r, g: rgb.g, b: rgb.b},
    //   }).toBuffer()
      var thirtyPercentGrey = String(Math.round(0.30 * 255))
      var suit_background_img = await sharp(suit_background)
            .png()
            .raw()
            .toBuffer()
